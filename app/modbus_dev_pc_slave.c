...
stModbus_RTU_Handler_Attr rtu_pc_attr_slave = {
    ...
    .ucReg_map_id = 0,
    .pSend_f = uart_pc_send,
    .pRecv_f = uart_pc_recv,
    .pRead_input_def = rtu_pc_read_input,
    .pRead_hold_def = rtu_pc_read_hold,
    .pWrite_hold_def = rtu_pc_write_hold,
    .pstFun_table = stRTU_fun_table_pc_slave,
    .ucFun_table_items = RTU_FUN_TABLE_ITEMS,
};
...
static int8_t rtu_pc_read_hold(stModbus_RTU_HoldReader_def *reader)
{
    ...
    {
        reader->ucReg_data_byte[i] = (uint8_t)(s_pc_hold_reg[reader->usReg_addr-RTU_PC_MASTER_HOLD_ADDR_MIN + i/2] >> 8);
        reader->ucReg_data_byte[i+1] = (uint8_t)s_pc_hold_reg[reader->usReg_addr-RTU_PC_MASTER_HOLD_ADDR_MIN + i/2];
    }
